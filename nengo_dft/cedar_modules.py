...
class GaussKernel(object):
    ...
    def __init__(self, c, sigma, normalize=True, dims=2):
        if dims != 0 and dims != 1:
            assert isinstance(sigma, list), f"Sigma has to be a list of sigma per dimension, but type is {type(sigma)}."
            assert len(sigma) == dims, f"Sigma should be given per dimension, but {len(sigma)} values are given for dimensionality {dims}."
        self.c = c
        self.sigma = sigma
        self.normalize = normalize
        # determine kernel width, which is 5 * sigma per dimension
        if dims == 1 or dims == 0:
            kernel_width = int(np.ceil(sigma*5)) # limit is always 5 
        elif dims == 2 or dims == 3:
            kernel_width = [int(np.ceil(s*5)) for s in sigma]
            kernel_width = [(w + 1) if (w % 2 == 0) else w for w in kernel_width]
...
class BoxKernel(object):
    ''' TODO: update docstring.
        ...
        dimensional fields.
    '''
    # TODO: Test box kernel, especially for higher dimensions
    def __init__(self, sizes, amplitude):
        self.sizes = sizes
        # dimensionality of BoxKernel always 0
        self.dims = len(sizes)
        self.amplitude = amplitude
        self.kernel_matrix = np.ones(sizes) * amplitude
...
#         # 5. no global_inh, no convolution, but recurrent
#         # self.u += (-self.u + self.h + stim)/self.tau + \
#         #           (self.input_noise_gain * np.random.randn(*self.sizes)) / self.tau 
#         # 4. no global_inh, no recurrent, but convolution
#         self.u = (-self.u + self.h + recurr + stim)/self.tau + \
#                   (self.input_noise_gain * np.random.randn(*self.sizes)) / self.tau 
#         # 3. no global_inh, no recurrent, no convolution
#         # self.u = (-self.u + self.h + stim)/self.tau + \
#         #           (self.input_noise_gain * np.random.randn(*self.sizes)) / self.tau 
#         # 2. no recurrent, no convolution
#         # self.u = (-self.u + self.h + self.c_glob * np.sum(a) + stim)/self.tau + \
#         #           (self.input_noise_gain * np.random.randn(*self.sizes)) / self.tau 
#         # 1. no recurrent
#         # self.u = (-self.u + self.h + self.c_glob * np.sum(a) + recurr + stim)/self.tau + \
#         #           (self.input_noise_gain * np.random.randn(*self.sizes)) / self.tau 
...
class Convolution(object):
    ''' Class around a nengo.Node that receives an input and performs
        a convolution on it, either with a static kernel passed at initialization or
        with a changing kernel that is also provided as an input to the node.

        Parameters
        ----------
        sizes : list of ints
            Size per dimension of the kernel and the input to convolve over
        border_type : {'zero-filled borders', 'cyclic'}
        ...
        ----------
        node : nengo.Node instance
            Node instance that performs the convolution. Only created 
            after calling method `make_node()`

        Notes
        -----
        The convolution node can be used for two use cases:
        1. Using a static convolution kernel. To use a static kernel, provide a kernel
           instance to the static_kernel parameter on initialization.
        2. Use a changing kernel. If you want to use a changing kernel, provide the
           size of the kernel as an initialization parameter. This will create a node
           instance of size Product(sizes) + Product(kernel_sizes). Provide the changing
           kernel as an input to the node by connecting it as follows:
                nengo.Connection(kernel, conv.node[-np.prod(conv.kernel_sizes):])
           Similarly, the input has to be connected as follows:
                nengo.Connection(input, conv.node[:np.prod(conv.sizes)])
           
        NOTE / TODO: Using a changing kernel with a different dimensionality than the
        input.
    '''
    def __init__(self, sizes, border_type, kernel_sizes=None, kernel=None, name=None):
        
        self.sizes = sizes
        self.border_type = border_type
        self.name = name
        assert not(kernel_sizes is None and kernel is None), "No kernel provided!"
        assert not (kernel is None and len(kernel_sizes) != len(sizes)), "Input and kernel need to have the same dimensionality. Other options not implemented yet."
    ...
    def update_cyclic(self, inp):
        ''' Update method for `border_type` cyclic
            ...
            ----------
            stim : array_like
                Input to the nengo.Node, kernel and matrix to convolve on
            ...
            array_like
                Result of the convolution
        '''
        # get kernel and input matrix for convolution
        if self.kernel_sizes is None:
            kernel = self.kernel.kernel_matrix
            matrix = inp.reshape(*self.sizes)
        else:
            kernel = inp[-int(np.prod(self.kernel_sizes)):].reshape(*self.kernel_sizes)
    ...
    def make_node(self):
        ''' Creates a nengo.Node() instance that takes care of the 
            Convolution computations. It uses one of the above update 
            methods for its update in each time step. 
        ''' 
        if self.border_type == 'zero-filled borders':
        ...
        size_in = int(np.prod(self.sizes) + np.prod(self.kernel_sizes)) if self.kernel is None else int(np.prod(self.sizes))
        if self.name is not None:
            self.node = nengo.Node(update, size_in=size_in, 
                          size_out=int(np.prod(self.sizes)), label=self.name)
        else:
            self.node = nengo.Node(update, size_in=size_in, 
                          size_out=int(np.prod(self.sizes)))
